...
interface ResultsAvailableTemplateProps {
...
export function renderResultsAvailableHtml(
  props: ResultsAvailableTemplateProps
): string {
  const { candidateName, totalScore, maxScore, dashboardUrl } = props
...
<head>
  ...
  <title>Results Available — ${FROM_NAME}</title>
</head>
<body style="
  margin: 0;
  padding: 0;
          ...
          <tr>
            <td style="padding: 24px 40px 0 40px;">
              <p style="
                ...
                color: #475569;
              ">
                Your assessment results are now available on ${FROM_NAME}. Here is your score summary:
              </p>
            </td>
          </tr>
          <tr>
                ...
                <tr>
                  <td style="padding: 24px; text-align: center;">
                    ...
                    <p style="
                      ...
                      font-weight: 700;
                      color: #4f46e5;
                    ">${totalScore}<span style="font-size: 18px; font-weight: 400; color: #94a3b8;">/${maxScore}</span></p>
                    <p style="
                      margin: 8px 0 0 0;
                      font-size: 14px;
                      font-weight: 600;
                      color: ${performanceColor};
                    ">${percentage}% — ${performanceLabel}</p>
                  </td>
                </tr>
              </table>
            </td>
...
export function renderResultsAvailableText(
  props: ResultsAvailableTemplateProps
): string {
  const { candidateName, totalScore, maxScore, dashboardUrl } = props
  const url = dashboardUrl || `${APP_URL}/dashboard`
...
Your assessment results are now available on ${FROM_NAME}.
